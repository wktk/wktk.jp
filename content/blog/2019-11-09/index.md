...
- 他の航空従事者を受験するときに学科試験の通信などの科目が免除される[^2]
...
一見無意味だが、航空通信士には種別や限定がないので、取得することで例えば回転翼の操縦士でも固定翼機で
通信ができるようになる。実用上はともかくとしても、操作範囲は広がる。

## 2019-05 申請書類の入手

申請書の様式は OCR 用のマークシート式になっていて、国土交通大臣の定める基準に適合するものを使う必要がある[^6]。
...
- 学科試験申請
  ...
  - 板紙封筒 -- 1 枚 (任意 -- 窓口持参なら不要 / 一般封筒でも不備にはならないはず)
- 交付申請
  - 第 19 号の 2 様式
  - 指定窓付封筒 -- 1 枚
  - 板紙封筒 -- 1 枚 (任意 -- 同上)
...
があれば航空通信士の申請には足りるが、めんどうだし特段安くなるわけでもないので、セットで買ったほうが不足がなくて良さそう。
...
というのも、所属先は航空とも今回の受験とも何も関係がないので書いても仕方がないし、
...
ということだったので、その場で「個人」と書いて電話番号は空欄にして提出した [^7] 。
...
従免を後から取る場合は、無線従事者試験は申込・受験・合格・交付で数ヶ月かかるうえ、実施回数も限られているため、早めに取らないと技能証明の交付申請の期限に間に合わないおそれがある。
...
問題はほとんどが過去問から出題される体感なので [^9]、これを中心に進めると大変効率がよい。
...
プログラムは黒い画面に抵抗がない人向けなのでともかく、CSV のデータは Excel や、
...
試験時間は 40 分で時間がかなり余る。
途中退室などはなく、余った 30 分の待機を 5 科目繰り返すので、これがわりとつらい。
...
ちなみに、学科試験には科目合格制度があり、**合格通知の日から** 1 年以内に実施される学科試験では合格済みの科目が免除される [^10]。
また、学科試験に全科目合格したときの交付申請の期限は、**学科試験を申請した日から** 2 年以内である [^11]。
...
かつて大型二種免許経由で無軌条電車 (= トロリーバス) 用などの免許が取得できた動力車操縦者運転免許 (= 鉄道の運転士資格) にも似ているところがある。
航空通信士は現代では使われず、国際民間航空条約でも資格を設けることが必須とされていないようなので、制度上の意義も希薄化していると思われる。
動力車操縦者と同様に、航空従事者技能証明を一般人が取得できるルートがなくなる可能性もあるかもしれない[^12]。
...
[^5]: 航空無線通信士か、航空無線通信士の操作範囲をを包含する第一級総合無線通信士・第二級総合無線通信士のいずれかが必要
[^6]: [航空法施行規則 239 条の 5 第 1 項]
[^7]: もっとも、届いた受験票の所属欄は空欄になっていたので、空欄でよいのだと思う。合格後の交付申請書 (19 号の 2 様式) の所属欄は空欄で出した。空欄として扱われるとしても、あえて「個人」「なし」「無所属」などと書くことで書き忘れではないと示す効果はあるかもしれない。あるいは、学生であれば学校名を書けば収まりがよいかもしれない。
[^8]: 段落冒頭のツイートの問題は誤りを選択する問題で、(3) は「9 時間 __遅い__ 」⇒「9 時間 __早い__ 」の部分が誤りだが、一般に UTC はグリニッジ天文台ではなく原子時計が基準なので (1) も間違いといえると考えた。ツイート内では GMT なら間違いないと書いたが、実は GMT もグリニッジ天文台からずれているということなので、どちらにせよ誤りと考えることもできる。ただ、日本の法令上はグリニッジ天文台の子午儀を本初子午線として扱っている事情もある。これ以上はわからないので、「誤りでないとは言い切れない」と迂遠な表現をしている。
[^9]: 残りが新問なのか覚えていないだけなのかはわからない
[^10]: [航空法施行規則 48条の2]
[^11]: [航空法施行規則 42条第5項]
[^12]: ドローン国家資格化の動きがあるので必ずしもそんなことはない。その際は航空通信士でも科目免除があると嬉しいけど、有人飛行と無人飛行はだいぶ性質が異なり、航空無線というよりは陸上無線であまり互換性がないので恐らくなんじゃないかと思う
